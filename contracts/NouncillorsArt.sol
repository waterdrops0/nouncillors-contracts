...
/// @title The Nouncillors art storage contract
...
contract NouncillorsArt is INouncillorsArt {
    /// @notice Current Nouncillors Descriptor address
    address public override descriptor;

    /// @notice Current inflator address
    IInflator public override inflator;

    /// @notice Nouncillor Backgrounds (Hex Colors)
    string[] public override backgrounds;

    /// @notice Nouncillor Color Palettes (Index => Hex Colors, stored as a contract using SSTORE2)
    mapping(uint8 => address) public palettesPointers;

    /// @notice Nouncillor Bodies Trait
    Trait public bodiesTrait;

    /// @notice Nouncillor Accessories Trait
    Trait public accessoriesTrait;

    /// @notice Nouncillor Heads Trait
    Trait public headsTrait;

    /// @notice Nouncillor Glasses Trait
    ...
    function addPage(
        ...
        trait.storagePages.push(
            NouncillorArtStoragePage({ pointer: pointer, decompressedLength: decompressedLength, imageCount: imageCount })
        );
     ...
     * @return INouncillorsArt.NouncillorArtStoragePage the page containing the image at index
     * @return uint256 the index of the image in the page
     */
    function getPage(INouncillorsArt.NouncillorArtStoragePage[] storage pages, uint256 index)
        internal
        view
        returns (INouncillorsArt.NouncillorArtStoragePage storage, uint256)
    {
        uint256 len = pages.length;
        uint256 pageFirstImageIndex = 0;
        for (uint256 i = 0; i < len; i++) {
            INouncillorsArt.NouncillorArtStoragePage storage page = pages[i];
    ...
    function decompressAndDecode(INouncillorsArt.NouncillorArtStoragePage storage page) internal view returns (bytes[] memory) {

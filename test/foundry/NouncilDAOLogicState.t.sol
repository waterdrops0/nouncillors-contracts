...
contract NouncilDAOLogicStateTest is Test, NouncilDAOLogicHelpers {
    ...
    ) internal virtual override returns (NouncilDAOLogic) {
        // Define vetoer or get it from the contract state if needed (assumed to be address(0) here)
        address vetoer = address(0); 

        NouncilDAOLogic daoProxy = _createDAOV3Proxy(
            timelock,
            nouncillorsToken,
            vetoer
        );
